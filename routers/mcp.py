...
TOOLS: list[dict] = [
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "output_format": {"type": "string", "enum": ["html", "markdown", "structured", "css"], "default": "markdown"},
...
def _rpc_error(message_id: Any, code: int, message: str) -> JSONResponse:
    return JSONResponse(status_code=200, content={"jsonrpc": "2.0", "id": message_id, "error": {"code": code, "message": message}})


def _text_result(message_id: Any, text: str, is_error: bool = False) -> JSONResponse:
    return JSONResponse(
        status_code=200,
        content={
            "jsonrpc": "2.0",
...
async def _dispatch(message_id: Any, method: str, params: dict | None) -> JSONResponse | None:
    params = params or {}
    ...
    if method == "notifications/initialized" or method == "notifications/cancelled":
        return None
    if method == "ping":
        return JSONResponse(status_code=200, content={"jsonrpc": "2.0", "id": message_id, "result": {}})
    if method == "tools/list":
        return JSONResponse(
            status_code=200,
            content={"jsonrpc": "2.0", "id": message_id, "result": {"tools": TOOLS}},
        )
    if method == "tools/call":
        name = (params or {}).get("name")
        arguments = (params or {}).get("arguments") or {}
        if not name:
            return _rpc_error(message_id, -32602, "Invalid params: missing tool name")
        try:
            payload = await _handle_tool_call(name, arguments)
        except Exception as e:
            return _text_result(message_id, f"Tool {name} failed: {e}", is_error=True)
        return _text_result(message_id, json.dumps(payload, ensure_ascii=False, default=str))
    if method == "resources/list":
        return JSONResponse(status_code=200, content={"jsonrpc": "2.0", "id": message_id, "result": {"resources": []}})
...
@router.get("/mcp", dependencies=[Depends(verify_api_key)])
async def mcp_sse(request: Request):
    """SSE endpoint advertising the message POST endpoint (legacy SSE transport)."""

    async def event_stream():
        yield f"event: endpoint\ndata: /mcp?sessionId=sse-{SERVER_INFO['name']}\n\n"
